 ...
 * -----                                                                                                               *
 * Last Modified: 2025.11.16, Sunday  09:53:14                                                                         *
 * Modified By: BT-Soft                                                                                                *
 * -----                                                                                                               *
 * HISTORY:                                                                                                            *
 * Date      	By	Comments                                                                                           *
    ...
    void layoutVerticalButtonGroup(const std::vector<UIButtonGroupDefinition> &buttonDefs, std::vector<std::shared_ptr<UIButton>> *out_createdButtons = nullptr,
        ...
        for (size_t colIndex = 0; colIndex < colsOfButtons.size(); ++colIndex) {
            ...
            for (const auto &def : currentCol) {
                ...
                auto button = std::make_shared<UIButton>(def.id, bounds, def.label, def.type, def.initialState, def.callback, UIColorPalette::createDefaultButtonScheme(), autoSizeBtn, def.initiallyDisabled);
