...
float SpectrumVisualizationComponent::getAdaptiveScale(float baseConstant) {
    ...
    float manualGainFactor = audioFftConfigAm;
...
bool SpectrumVisualizationComponent::isAutoGainMode() {
    float currentConfig = audioFftConfigAm;
    ...
    if (modeToPrepareFor != DisplayMode::Off) {
        ...
        if (bounds.width > 0 && graphH > 0) {
            ...
            if (spriteCreated_) {
                ...
                DEBUG("SpectrumVisualizationComponent: Sprite létrehozása sikertelen, mód: %d (szélesség:%d, grafikon magasság:%d)\n", static_cast<int>(modeToPrepareFor), bounds.width, graphH);
            }
        }
    }
...
void SpectrumVisualizationComponent::setFftParametersForDisplayMode() {
    ...
    if (sdBack.displayMinFreqHz != 0 || sdBack.displayMaxFreqHz != 0) {
        sdToUse = &sdBack;
        // DEBUG("SpectrumVisualizationComponent: hátsó-buffer hasznalata (idx=%d)\n", backIdx);
    } else if (sdActive.displayMinFreqHz != 0 || sdActive.displayMaxFreqHz != 0) {
        sdToUse = &sdActive;
        // DEBUG("SpectrumVisualizationComponent: aktív-buffer hasznalata (idx=%d)\n", activeIdx);
    }
...
void SpectrumVisualizationComponent::loadModeFromConfig() {
    ...
    uint8_t configValue = audioModeAM;
...
void SpectrumVisualizationComponent::setTuningAidType(TuningAidType type) {
    ...
    if (currentMode_ == DisplayMode::CWWaterfall      //
        ...
        if (currentTuningAidType_ == TuningAidType::CW_TUNING) {
            ...
            uint16_t centerFreq = cwToneFrequencyHz;
            ...
            uint16_t f_mark = rttyMarkFrequencyHz;
            uint16_t f_space = f_mark - rttyShiftHz;
 ...
 */
void SpectrumVisualizationComponent::updateTuningAidParameters() {
        ...
        if (displayed_span_hz > 0) {
            if (currentTuningAidType_ == TuningAidType::CW_TUNING) {
                ...
                if (currentTuningAidMaxFreqHz_ > currentTuningAidMinFreqHz_) {
                    ...
                    float ratio_center = (static_cast<float>(cwToneFrequencyHz) - static_cast<float>(currentTuningAidMinFreqHz_)) / span;
                    ...
                    sprite_->drawString(String(cwToneFrequencyHz) + "Hz", line_x, label_y);
                }
...
void SpectrumVisualizationComponent::renderSnrCurve() {
    ...
    if (MIN_FREQ_HZ == 0 || MAX_FREQ_HZ == 0 || MIN_FREQ_HZ >= MAX_FREQ_HZ) {
        ...
        if (Utils::timeHasPassed(lastSnrErrorDebugTime, 10000)) {
            DEBUG("SpectrumVisualizationComponent::renderSnrCurve - Érvénytelen frekvencia határok: MIN=%.2f, MAX=%.2f, automatikus javítás!\n", MIN_FREQ_HZ, MAX_FREQ_HZ);
    ...
    if (debugOnce) {
        DEBUG("SpectrumVisualizationComponent::renderSnrCurve - Aktuális frekvenciahatárok: MIN=%.2f, MAX=%.2f\n", MIN_FREQ_HZ, MAX_FREQ_HZ);
        debugOnce = false;
    }
    ...
    if (displayed_span_hz > 0) {
        ...
        if (currentMode_ == DisplayMode::CwSnrCurve) {
            ...
            uint16_t cwFrequency = cwToneFrequencyHz;
 ...
 */
bool SpectrumVisualizationComponent::getCore1SpectrumData(const int16_t **outData, uint16_t *outSize, float *outBinWidth, float *outAutoGain) {
    ...
    if (activeSharedDataIndex < 0 || activeSharedDataIndex > 1) {
        ...
        *outSize = 0;
        ...
        DEBUG("SpectrumVisualizationComponent::getCore1SpectrumData - érvénytelen shared index: %d\n", activeSharedDataIndex);
        return false;
    }
 ...
 */
bool SpectrumVisualizationComponent::getCore1OscilloscopeData(const int16_t **outData, uint16_t *outSampleCount) {
    // Core1 oszcilloszkóp adatok lekérése
    int8_t activeSharedDataIndex = audioController.getActiveSharedDataIndex();
    if (activeSharedDataIndex < 0 || activeSharedDataIndex > 1) {
        *outData = nullptr;
        *outSampleCount = 0;
        DEBUG("SpectrumVisualizationComponent::getCore1OscilloscopeData - érvénytelen shared index: %d\n", activeSharedDataIndex);
        return false;
    }
 ...
 */
double SpectrumVisualizationComponent::getInterpolatedMagnitude(const int16_t *magnitudeData, float exactBinIndex, int minBin, int maxBin) const {
