#!/usr/bin/env python3
"""
resample_wav_to_40khz.py
Átkonvertál WAV fájlokat 40000 Hz mintavételre a WEFAX dekóderhez.
(CPU-barát mintavétel, elég a WEFAX max 2.3 kHz jelekhez)

Használat:
    python resample_wav_to_40khz.py input.wav [output.wav]
    
Ha nincs megadva output fájl, akkor input-40k.wav lesz a kimenet.
"""
import sys
import numpy as np
...
def resample_to_40khz(input_path, output_path=None):
    """
    Átkonvertál egy WAV fájlt 40000 Hz mintavételre.
    ...
    if output_path is None:
        output_path = input_path.parent / f"{input_path.stem}-40k{input_path.suffix}"
    else:
    ...
    # Resample 40 kHz-re (CPU-barát, elég a WEFAX-hoz)
    target_sr = 40000
    
    if sr == target_sr:
        print(f"  ⚠️ Már 40000 Hz mintavétel, másolás...")
...
def main():
    if len(sys.argv) < 2:
        print("Használat: python resample_wav_to_40khz.py input.wav [output.wav]")
        print()
        print("Példák:")
        print("  python resample_wav_to_40khz.py phase-sample.wav")
        print("  python resample_wav_to_40khz.py input.wav output-40k.wav")
        sys.exit(1)
    ...
    try:
        resample_to_40khz(input_path, output_path)
